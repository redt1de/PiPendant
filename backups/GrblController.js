...
export default class GrblController extends EventEmitter {
    ...
    notifyListeners() {
        this.listeners.forEach(callback => callback(this.machineState, this.consoleMessages, this.probeHistory));
    }
    ...
    parseData(line) {
        ...
        if (line.startsWith("<")) {
            ...
            if (line.startsWith("[PRB:")) {
                ...
                this.machineState.activeCS = modalUpdate.coordinateSystem || "G54";
                this.emit("machineState", this.machineState);
                ...
                this.parseToolLengthOffset(line);
                this.emit("machineState", this.machineState);
            }
    ...
    parseStatus(line) {
        ...
        attributes.forEach(attr => {
            if (attr.startsWith("MPos:")) {
                ...
                statusUpdate.mpos = { x: Number(x), y: Number(y), z: Number(z) };
    ...
    updateWorkPosition() {
        ...
        const offset = offsets[activeCS] || { x: 0, y: 0, z: 0 };
    ...
    parseToolLengthOffset(line) {
        ...
        if (match) {
            this.machineState.tlo = Number(match[1]).toFixed(3);
        }
    }
}
